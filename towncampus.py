...
def show_towncampus_page():
    # Streamlit app
    st.title("Town Campus(Nairobi CBD)")
    ...
    town_campus_df = df[df['Campus'] == "TOWN"]
    
    # Count unique Registration No. based on Cohorts
    town_cohort_counts = town_campus_df.groupby('Cohorts')['Registration No'].nunique().reset_index()
    # Rename columns for clarity
    town_cohort_counts.columns = ['Cohorts', 'Student Numbers']
    
    # Function to plot the bar graph for Student Number Counts by Cohorts
    def plot_unique_registration_no_count(town_cohort_counts):
        fig, ax = plt.subplots(figsize=(12, 8))
        bars = ax.bar(town_cohort_counts['Cohorts'], town_cohort_counts['Student Numbers'], color='skyblue')
        ax.set_title('Student Numbers by Cohorts')
        ax.set_xlabel('Cohorts')
        ax.set_ylabel('Student Numbers Count')
        ax.set_xticklabels(town_cohort_counts['Cohorts'], rotation=45)
        ax.grid(axis='y')
    ...
    def plot_cohorts_gender_count():
        ...
        df_gender_count = town_campus_df.groupby(['Cohorts', 'Gender']).size().reset_index(name='Count')
    ...
    if option == 'Student Numbers by Cohorts':
        st.dataframe(town_cohort_counts)
        plot_unique_registration_no_count(town_cohort_counts)
